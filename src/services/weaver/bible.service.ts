...
function activeSlots(facts: WeaverCommittedFact[]): SpineSlot[] {
  const covered = new Set(facts.filter((f) => f.fact.trim()).map((f) => f.slot));
  return SPINE_SLOTS.filter((s) => !s.optional || covered.has(s.id));
}

export function slotsToAuthor(facts: WeaverCommittedFact[]): SpineSlot[] {
  const covered = new Set(facts.map((f) => f.slot));
  return SPINE_SLOTS.filter((s) => !covered.has(s.id) && !s.optional);
}

export function partsToAuthor(facts: WeaverCommittedFact[]): SynthesisTarget[] {
  const covered = new Set(facts.filter((f) => f.fact.trim()).map(factKey));
  const out: SynthesisTarget[] = [];
  for (const slot of activeSlots(facts)) {
    const hybrid = isHybrid(slot);
    for (const part of slotParts(slot.id)) {
...
export function coerceAuthoredParts(value: unknown, authorable: Set<string>): AuthoredPart[] {
  ...
  for (const raw of value) {
    ...
    const slot = r.slot;
    if (typeof slot !== "string" || !isSlotId(slot)) continue;
...
function coerceLinks(value: unknown): WeaverBibleCausalLink[] {
  if (!Array.isArray(value)) return [];
  ...
  for (const raw of value) {
    ...
    if (!isSlotId(from) || !isSlotId(to) || from === to) continue;
...
export function assembleSpine(
  ...
  const authorable = new Set(partsToAuthor(facts).map((t) => `${t.slot}:${t.part}`));
  const authoredBy = new Map(
    coerceAuthoredParts(authoredRaw, authorable).map((a) => [`${a.slot}:${a.part}`, a.content]),
  );

  const factContent = new Map<string, string[]>();
  for (const f of facts) {
    if (!f.fact.trim() || !isSlotId(f.slot)) continue;
    const key = factKey(f);
  ...
  return { entries, causal_links: coerceLinks(linksRaw), brief };
}
...
function rowToBible(row: Record<string, unknown>): WeaverBible {
  const spine = parseSpine(row.spine);
  return {
    session_id: row.session_id as string,
    spine,
    status: parseStatus(row.status),
    gate: parseGate(row.gate, spine),
...
function parseSpine(value: unknown): WeaverBibleSpine {
  ...
  const entries = Array.isArray(obj.entries)
    ? obj.entries
        .filter((e): e is Record<string, unknown> => {
          ...
          return isSlotId(r.slot) && typeof r.content === "string";
        })
        .map((e) => {
    ...
    : [];
  const causal_links = coerceLinks(obj.causal_links);
  const brief = typeof obj.brief === "string" ? obj.brief.trim() : "";
  return { entries, causal_links, brief };
}
...
function parseGate(value: unknown, spine: WeaverBibleSpine): WeaverGateVerdict | null {
  const obj = parseObject(value);
  if (!Array.isArray(obj.criteria)) return null;
  return buildGateVerdict(obj, applicableBibleCriteria(spine));
}
...
export async function synthesizeBible(
  ...
  const targets = partsToAuthor(facts);

  let usage = emptyUsage();
  const authoredAll: AuthoredPart[] = [];
  for (const group of SYNTHESIS_GROUPS) {
    const groupTargets = targets.filter((t) => slotSynthesisGroup(t.slot) === group.id);
    if (groupTargets.length === 0) continue;
    const res = await weaverGenerateJsonWithUsage({
      userId,
      session,
      system: buildBibleSynthesisPrompt(group),
      user: buildBibleSynthesisUserMessage({
        facts,
        taste,
    ...
    authoredAll.push(...coerceAuthoredParts(res.data.authored, authorable));
    usage = addUsage(usage, res.usage);
  }

  const partial = assembleSpine(facts, authoredAll, [], "");
  ...
  const weaveRes = await weaverGenerateJsonWithUsage({
    userId,
    session,
    system: buildBibleWeavePrompt(),
    user: buildBibleWeaveUserMessage(partial),
    temperature: 0.5,
    signal,
  });
  usage = addUsage(usage, weaveRes.usage);

  const spine = assembleSpine(facts, authoredAll, weaveRes.data.causal_links, weaveRes.data.brief);

  const applicable = applicableBibleCriteria(spine);
  const gateRes = await weaverGenerateJsonWithUsage({
    userId,
    session,
    system: buildBibleGatePrompt(applicable),
    user: buildBibleGateUserMessage(spine),
    temperature: 0.2,
    signal,
  });
...
export async function gateBible(
  ...
  const applicable = applicableBibleCriteria(bible.spine);
  const gateRes = await weaverGenerateJsonWithUsage({
    userId,
    session,
    system: buildBibleGatePrompt(applicable),
    user: buildBibleGateUserMessage(bible.spine),
    temperature: 0.2,
    signal,
  });
  const verdict = buildGateVerdict(gateRes.data, applicable);
...
export function updateBible(
  ...
  const entries =
    input.entries === undefined
      ? bible.spine.entries
      : input.entries
          .filter((e) => isSlotId(e.slot) && typeof e.content === "string" && e.content.trim())
          .map((e) => {
            const entry: WeaverBibleEntry = {
              slot: e.slot,
  ...
  const causal_links =
    input.causal_links === undefined ? bible.spine.causal_links : coerceLinks(input.causal_links);

  const brief = input.brief === undefined ? bible.spine.brief : input.brief.trim();

  const spine: WeaverBibleSpine = { entries, causal_links, brief };
